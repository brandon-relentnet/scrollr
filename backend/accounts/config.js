...
export const sportsConfig = {
  ...
  logFilePath: process.env.LOG_FILE_PATH || "./logs/accounts.log",
};
...
export function validateConfig(serviceName) {
  ...
  if (serviceName === "accounts" && process.env.JWT_SECRET) {
    ...
    if (
      insecureSecrets.includes(process.env.JWT_SECRET) ||
      ...
      console.error(
        "Current JWT_SECRET is too weak and must be changed before production deployment."
      );
