...
function getSelectedRssFeeds(rssState) {
    ...
    return rssState.feeds.filter(feed => 
        rssState.customSelections[feed.id] === true
    );
}
...
async function fetchRssFeed(feed) {
    ...
    for (let i = 0; i < CORS_PROXIES.length; i++) {
        try {
            ...
            console.log(`Attempting to fetch RSS feed "${feed.name}" via proxy ${i + 1}`);
            ...
            if (i === 0) { // corsproxy.io returns raw content
                xmlText = await response.text();
            } else if (i === 1) { // allorigins.win returns JSON with contents
                const data = await response.json();
                xmlText = data.contents;
            } else { // proxy.cors.sh returns raw content
                xmlText = await response.text();
            }
            ...
            // Extract RSS items
            const items = Array.from(xmlDoc.querySelectorAll('item, entry')).slice(0, 10); // Limit to 10 items per feed
            ...
            console.log(`Successfully fetched ${parsedItems.length} items from RSS feed "${feed.name}" via proxy ${i + 1}`);
            return parsedItems;
            
        } catch (error) {
            lastError = error;
            console.warn(`Proxy ${i + 1} failed for RSS feed "${feed.name}":`, error.message);
    ...
    console.error(`All CORS proxies failed for RSS feed "${feed.name}":`, lastError);
    return [];
}
...
export default function useRssData() {
    ...
    }, [isAuthenticated, rssState?.enabled, rssState?.feeds?.length, rssState?.customSelections]);
    ...
    }, [hasActiveRssFeeds, selectedFeeds.length, rssItems.length, connectionStatus, isLoading, isInitialized]);
