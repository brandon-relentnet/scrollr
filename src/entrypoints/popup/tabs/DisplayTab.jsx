...
export default function DisplayTab() {
    ...
    const [rssFormData, setRssFormData] = useState({ name: '', url: '', category: 'General' });
    ...
    // RSS handlers
    const handleRssSubmit = useCallback(async (e) => {
        e.preventDefault();
        ...
        <>
                    ...
                    <fieldset className="fieldset bg-base-100 border-base-300 space-y-2 rounded-box w-full border p-4">
                        ...
                        {!isAuthenticated ? (
                            <div className="text-center py-4">
                                <p className="text-base-content/70 mb-3">Login to manage your RSS feeds</p>
                                <button 
                                    className="btn btn-primary btn-sm"
                                    onClick={() => {
                                        // Switch to accounts tab
                                        const accountsTab = document.querySelector('input[aria-label="Tab 4"]');
                            ...
                            <>
                                <div className="space-y-2">
                                    <label className={`${!rssState?.enabled ? 'text-base-content/50' : 'text-base-content'} .label btn btn-ghost justify-between flex items-center`}>
                                        📡 RSS Feeds
                                        <input
                                            type="checkbox"
                                            className="toggle"
                                        ...
                                        <div className="space-y-2 p-2 ml-2 border-l border-base-300/50">
                                            <form onSubmit={handleRssSubmit} className="space-y-2">
                                                <label className="floating-label">
                                                    ...
                                                    <input
                                                        ...
                                                        onChange={(e) => handleRssInputChange('name', e.target.value)}
                                                    />
                                                </label>
                                                <label className="floating-label">
                                                    ...
                                                    <input
                                                        ...
                                                        onChange={(e) => handleRssInputChange('url', e.target.value)}
                                                    />
                                                </label>
                                                <label className="floating-label">
                                                    <span>Category</span>
                                                    <select
                                                        ...
                                                        onChange={(e) => handleRssInputChange('category', e.target.value)}
                                                    >
                                                ...
                                                <div className="flex gap-2">
                                                    <button 
                                                        ...
                                                        disabled={isLoading || !rssFormData.name || !rssFormData.url}
                                                    >
                                                    ...
                                                    >
                                                        Select ({Object.values(rssState?.customSelections || {}).filter(Boolean).length})
                                                    </button>
                                                </div>
                                            </form>
                                            ...
                                            {feeds.length > 0 && (
                                                <div className="text-xs text-base-content/50">
                                                    {feeds.length} feed{feeds.length !== 1 ? 's' : ''} in collection
                                                </div>
                                            )}
                                        </div>
                                    )}
            ...
            <dialog id="rss_modal" className="modal">
                <div className="modal-box max-w-2xl">
                    ...
                    <div className="flex gap-2 mb-4">
                        <button className="btn btn-sm btn-outline" onClick={() => dispatch(toggleAllRssSelections(true))}>Select All</button>
                        <button className="btn btn-sm btn-outline" onClick={() => dispatch(toggleAllRssSelections(false))}>Deselect All</button>
                        <button className="btn btn-sm btn-ghost" onClick={() => dispatch(resetRssSelections())}>Reset to Default</button>
                        {rssState?.searchTerm && (
                            <button className="btn btn-sm btn-ghost" onClick={() => dispatch(setRssSearch(''))}>Clear Search</button>
                        )}
                    </div>
                        ...
                        {feeds.length === 0 ? (
                            ...
                            feeds
                                .filter(feed => {
                                    ...
                                    return feed.name.toLowerCase().includes(searchTerm) || 
                                           feed.category.toLowerCase().includes(searchTerm);
                                })
                                .map((feed) => (
                                    <div key={feed.id} className={`${rssState.customSelections?.[feed.id] ? 'bg-base-200' : 'bg-base-200/50'} p-3 rounded-lg flex items-center justify-between`}>
                                        <label className="cursor-pointer flex items-center gap-3 flex-1">
                                            ...
                                            <div className="flex flex-col items-start">
                                                ...
                                                <span className="text-sm text-base-content/70">{feed.category}</span>
                                                <span className="text-xs text-base-content/50 break-all">{feed.url}</span>
                                            </div>
                                        </label>
                                        <button
                    ...
                    <div className="text-sm text-base-content/70 mt-4 flex justify-between">
                        <span>{Object.values(rssState?.customSelections || {}).filter(Boolean).length} of {feeds.length} RSS feeds selected</span>
                        {rssState?.searchTerm && (
                            <span>Showing {feeds.filter(feed => {
                                const searchTerm = (rssState?.searchTerm || '').toLowerCase();
                                return feed.name.toLowerCase().includes(searchTerm) || 
                                       feed.category.toLowerCase().includes(searchTerm);
                            }).length} results</span>
