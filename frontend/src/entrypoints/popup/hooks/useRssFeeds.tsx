...
export function useRssFeeds() {
    ...
    // Add new RSS feed
    const addFeed = useCallback(async (feedData: RssFeedInput): Promise<{ success: boolean; error?: string }> => {
        if (!isAuthenticated || !token) {
    ...
    // Update RSS feed
    const updateFeed = useCallback(async (feedId: number, updates: Partial<RssFeedInput>): Promise<{ success: boolean; error?: string }> => {
        if (!isAuthenticated || !token) {
            return { success: false, error: 'Authentication required' };
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                return { success: false, error: data.error || 'Failed to update RSS feed' };
            }
        } catch (err) {
    ...
    // Delete RSS feed
    const deleteFeed = useCallback(async (feedId: number): Promise<{ success: boolean; error?: string }> => {
        if (!isAuthenticated || !token) {
            return { success: false, error: 'Authentication required' };
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                return { success: false, error: data.error || 'Failed to delete RSS feed' };
            }
        } catch (err) {
